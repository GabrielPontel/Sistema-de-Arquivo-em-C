...
// Inicializa todos os arquivos necess�rios ao sistema de arquivos.
...
void mkdir_(int num_ant, char *nome_arq)
{
    ...
    if (validar_nome_arquivo(nome_arq) && verificar_nome_diretorio(num_ant, nome_arq))
    {
        ...
        if (valida_inode(inode) && valida_novo_bloco(inode.blocos[0]))
        {
            ...
            // Grava nesse novo diret�rio as entradas . e .. .
            ...
            // Grava essa nova entrada de diret�rio no diret�rio atual.
            valida = gravar_entrada_diretorio_pai(num_ant, entrada_diretorio);
            // Se o diret�rio atual estiver cheio deve liberar o inode e o bloco.
            if (valida == -1)
            {
...
// Cria um novo arquivo dentro do diret�rio atual.
void touch(int num_pai, char nome_arquivo[])
{
    ...
    if (validar_nome_arquivo(nome_arquivo) && verificar_nome_diretorio(num_pai, nome_arquivo))
    {
        ...
        if (valida_inode(inode))
        {
            ...
            else
            {
                ...
                if (valida == 1)
                {
                    ...
                    // Com a aloca��o dos blocos efetuada, grava o conteudo em cada bloco.
                ...
                // Caso n�o foi possivel alocar todos os blocos os que tinha sido reservados seram liberados novamente.
...
// Separa os diret�rios em um vetor de string.
...
// Entrar em um diret�rio a partir do diret�rio atual.
int cd(int num_inode, char caminho[], char comando[])
{
    ...
    // Se o usuario deseja voltar ao diret�rio anterior.
    if (strcmp(comando, "..") == 0)
    {
    ...
    // Se o usuario deseja voltar ao diret�rio raiz.
...
// Apresenta a localiza��o atual dentro do sistema de arquivo.
void pwd(int num_inode_atual)
{
    ...
    raiz = ler_todas_entradas_inode(inode_atual.num);
    ...
    while (atual != NULL && pai != NULL && atual->info.num_inode != pai->info.num_inode)
    {
        ...
        raiz_pai = ler_todas_entradas_inode(num_pai);
...
// Apresentar o conteudo de um arquivo no diret�rio atual.
void cat(int num_diretorio_atual, char nome_arquivo[])
{
    ...
    raiz = ler_todas_entradas_inode(num_diretorio_atual);
...
// Exclui um arquivo ou diret�rio do diret�rio atual.
void rm(int inode_pai, char nome_arq[])
{
    ...
    raiz = ler_todas_entradas_inode(inode_pai);
    desejado = pesquisa_entrada_diretorio(raiz, nome_arq);
    if (desejado != NULL)
    {
        // Verifica se o nome arquivo n�o � . e .. e verifica se � arquivo ou diret�rio para ser excluido.
...
// Apresenta ao usu�rio as op��es que o programa contem.
void help()
{
